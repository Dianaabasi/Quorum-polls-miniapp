...
export default function ProfilePage() {
  ...
  const [stats, setStats] = useState({ votesCast: 0, usdEarned: 0, pollsCreated: 0 })
    ...
    if (isConnected && address) {
      const fetchStats = async () => {
        try {
          ...
          setStats({ votesCast: votedPolls, usdEarned: 0, pollsCreated })
        } catch (error) {
  ...
  const finalAvatarUrl = pfpUrl || fallbackAvatar
        ...
        <Card className="bg-card border-border p-8">
          <div className="flex flex-col items-center text-center mb-8">
            <div className="w-32 h-32 rounded-full overflow-hidden bg-muted mb-4 ring-4 ring-primary/20">
              <img
                src={finalAvatarUrl || "/placeholder.svg"}
                alt={displayName}
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="flex flex-col items-center text-center p-4 rounded-lg bg-background/50">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-2xl font-bold text-foreground">
                    {statsLoading ? "..." : stats.pollsCreated}
                  </span>
                </div>
